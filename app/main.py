...
async def process_facial_image(request: ProcessRequest, background_tasks: BackgroundTasks):
    """
    ...
    try:
        ...
        REQUEST_DURATION.labels('POST', '/api/v1/frontal/crop/submit').observe(time.time() - start_time)
...
async def get_job_status(job_id: str):
    ...
    if job_data["status"] == "pending":
        ...
        if task.ready():
            if task.successful():
                ...
                return JobStatusResponse(id=job_id, status="completed", result=task.result)
            else:
    ...
    return JobStatusResponse(id=job_id, status=job_data["status"], result=job_data.get("result"))
